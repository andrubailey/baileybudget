      ...
      <table className="w-full text-left">
        <thead>
          <tr className="border-b border-border bg-bg">
            ...
            {periods.map((p) => (
              <th key={p.id} className="px-4 py-2 text-xs font-medium whitespace-nowrap text-text-muted">
                {p.name}
              </th>
            ))}
            <th className="px-4 py-2 text-xs font-medium whitespace-nowrap text-text-muted">Total</th>
          </tr>
        </thead>
        <tbody>
          {[...rows]
            .sort(
              (a, b) =>
                (a.category.group_name ?? "").localeCompare(b.category.group_name ?? "") ||
            ...
            const showGroupHeader =
              row.category.group_name && sorted[i - 1]?.category.group_name !== row.category.group_name;
            return (
          ...
          {rows.length === 0 && (
            <tr>
              <td colSpan={periods.length + 2} className="px-6 py-10 text-center text-sm text-text-muted">
