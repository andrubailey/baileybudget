...
import { getCategoryProgress, getCategoriesForPeriod } from "@/lib/queries";
import { createCategory, updateCategoryPlanned } from "@/app/actions";
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-black/60 dark:text-white/60">
            Expense categories belong to the period shown below — add them
            fresh each period, with their own planned amount.
          </p>
        </div>
        ...
        <>
          ...
          >
            ...
            <div className="sm:col-span-4">
              ...
              <p className="mt-1 text-xs text-black/50 dark:text-white/50">
                Planned amount only applies to expense categories — income
                categories are shared across every period.
              </p>
            </div>
          </form>
                    ...
                    <td className="px-4 py-2">
                      <form
                        action={async (formData: FormData) => {
                          "use server";
                          const amount = Number(
                            formData.get("planned_amount") ?? 0,
                          );
                          await updateCategoryPlanned(c.id, amount);
                        }}
                ...
                {expenseProgress.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-6 text-center text-black/50 dark:text-white/50">
                      No expense categories for {period.name} yet.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div>
            <h2 className="text-sm font-medium text-black/70 dark:text-white/70">
              Income categories (shared across all periods)
            </h2>
