...
export function RecurringTable({
  ...
  function status(r: RecurringTransaction): { label: string; className: string } {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm text-text-muted">
          Generate this period&apos;s ({currentPeriodName}) transactions from your recurring list.
        </p>
        <button
          type="button"
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-border bg-bg">
              <th className="px-6 py-2 text-xs font-medium text-text-muted">Description</th>
              <th className="px-6 py-2 text-xs font-medium text-text-muted">Amount</th>
              <th className="px-6 py-2 text-xs font-medium text-text-muted">Day</th>
              ...
              <th className="px-6 py-2 text-xs font-medium text-text-muted">Active</th>
              <th className="px-6 py-2 text-xs font-medium text-text-muted" />
            </tr>
          </thead>
          <tbody>
            {items.map((r) => {
              ...
              >
                ...
                <td className="tabular px-6 py-3 text-sm text-text-muted">{r.day_of_month}</td>
                <td className="px-6 py-3 text-sm text-text-muted">{accountName(r.account_id)}</td>
                <td className="px-6 py-3 text-sm text-text-muted">{categoryName(r.category_id)}</td>
                ...
                <tr className="border-b border-border bg-bg/40 last:border-b-0">
                  <td colSpan={8} className="px-6 py-3">
                    {priceHistory.length === 0 ? (
                      <p className="text-xs text-text-muted">No generated transactions yet for this bill.</p>
                    ) : (
                          ...
                          return (
                            <span
                              ...
                              className={`rounded-md px-2 py-1 text-xs font-medium ${
                                changed ? "bg-[#fef0c7] text-[#93370d]" : "bg-bg text-text-muted"
                              }`}
                              title={formatDate(p.txn_date)}
                            >
                              {formatDate(p.txn_date)}: {formatMoney(p.amount)}
                            </span>
                          );
                        })}
                      </div>
