...
// 50/30/20-style breakdown: Needs and Wants split from expense categories'
// is_need flag, Savings is whatever's left of income after both.
export type NeedsWantsSavings = {
  needs: number;
  wants: number;
  savings: number;
  income: number;
...
export async function getNeedsWantsSavings(
  start: string,
  end: string,
): Promise<NeedsWantsSavings> {
  const supabase = await createClient();

  const [{ data: categories }, { data: transactions }] = await Promise.all([
    supabase.from("categories").select("id, is_need").eq("kind", "expense"),
    supabase
      .from("transactions")
  ...
  const isNeedByCategory = new Map<string, boolean>();
  for (const c of categories ?? []) {
    isNeedByCategory.set(c.id, c.is_need);
  }
  ...
  for (const t of transactions ?? []) {
    ...
    const isNeed = t.category_id ? (isNeedByCategory.get(t.category_id) ?? false) : false;
    if (isNeed) needs += t.amount;
    else wants += t.amount;
  }

  return { needs, wants, savings: income - needs - wants, income };
}
